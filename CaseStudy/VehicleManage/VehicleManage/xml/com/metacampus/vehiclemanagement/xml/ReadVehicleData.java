...
/**
 * Name: ReadVehicleData
 * @author Anurag
 * Since: 21 September,2015
 * Description: Reads the Information from Vehicle Table
 */
public class ReadVehicleData 
{
	// Updating the value to update in database
	 ...
	 * @param configFile
	 * @return
	 * Description: Reads the data of the Vehicle
	 */
	public List<Vehicle> readData(String configFile)
	{
		...
		try 
		{
			...
			XMLEventReader eventReader = inputFactory
					.createXMLEventReader(inputStreamObj);
			// read the XML document
			Vehicle vehicle = null;
			while (eventReader.hasNext())
			{
				XMLEvent event = eventReader.nextEvent();
				// Checks for each event and allots the value
				if (event.isStartElement())
				{
					...
					if (flag == true) 
					{
						...
						{
							event = eventReader.nextEvent();
							((Car) vehicle).setAC(event.asCharacters()
									.getData());
							continue;
						}
						if (event.asStartElement().getName().getLocalPart().equals(POWER_STEERING)) 
						{
							event = eventReader.nextEvent();
							((Car) vehicle).setPowerSteering(event
									.asCharacters().getData());
							continue;
						}
						if (event.asStartElement().getName().getLocalPart().equals(ACCESSORY_KIT))
						{
							event = eventReader.nextEvent();
							((Car) vehicle).setAccessoryKit(event
									.asCharacters().getData());
							continue;
						}
					}
					...
					{
						if (event.asStartElement().getName().getLocalPart().equals(SELF_START))
						{
							event = eventReader.nextEvent();
